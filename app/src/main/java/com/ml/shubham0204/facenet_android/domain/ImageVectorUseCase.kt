...
class ImageVectorUseCase(
    ...
    // Process the camera frame to detect faces and perform spoofing detection
    suspend fun detectFaces(
        frameBitmap: Bitmap
    ): Pair<RecognitionMetrics?, List<FaceDetectionResult>> {
        // Perform face detection and get cropped faces
        val (faceDetectionResult, t1) =
            measureTimedValue { mediapipeFaceDetector.getAllCroppedFaces(frameBitmap) }
        ...
        for (result in faceDetectionResult) {
            ...
            val spoofResult = faceSpoofDetector.detectSpoof(frameBitmap, boundingBox)
            avgT4 += spoofResult.timeMillis
            faceDetectionResults.add(FaceDetectionResult(boundingBox, spoofResult))
        }
        ...
        val metrics =
            if (faceDetectionResult.isNotEmpty()) {
                RecognitionMetrics(
                    timeFaceDetection = t1.toLong(DurationUnit.MILLISECONDS),
                    timeFaceEmbedding = 0L, // No embedding calculation
                    timeVectorSearch = 0L, // No database search
                    timeFaceSpoofDetection = avgT4 / faceDetectionResult.size
                )
            } else {
                null
        ...
        return Pair(metrics, faceDetectionResults)
    }
}